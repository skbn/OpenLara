...
int32 sphereIsVisible_c(int32 sx, int32 sy, int32 sz, int32 r)
{
    ...
    if (rMinX > viewport.x1 || rMaxX < viewport.x0 || rMinY > viewport.y1 || rMaxY < viewport.y0)
...
extern "C" X_NOINLINE void rasterize_c(uint32 flags, VertexLink *top)
{
    ...
    switch (type)
    {
    case FACE_TYPE_SHADOW:
        rasterizeS((uint16 *)pixel, top, R);
        break;
    case FACE_TYPE_F:
        rasterizeF((uint16 *)pixel, top, R);
        break;
    case FACE_TYPE_FT:
        rasterizeFT((uint16 *)pixel, top, R);
        break;
    case FACE_TYPE_FTA:
        rasterizeFTA((uint16 *)pixel, top, R);
        break;
    case FACE_TYPE_GT:
        rasterizeGT((uint16 *)pixel, top, R);
        break;
    case FACE_TYPE_GTA:
        rasterizeGTA((uint16 *)pixel, top, R);
        break;
    case FACE_TYPE_SPRITE:
        rasterizeSprite((uint16 *)pixel, top, R);
        break;
    case FACE_TYPE_FILL_S:
        rasterizeFillS((uint16 *)pixel, top, R);
        break;
    case FACE_TYPE_LINE_H:
        rasterizeLineH((uint16 *)pixel, top, R);
        break;
    case FACE_TYPE_LINE_V:
        rasterizeLineV((uint16 *)pixel, top, R);
        break;
    }
}
        ...
        do
        {
            ...
            if (type <= FACE_TYPE_GTA)
            {
                ...
                else
                {
                    ...
                    if (top->v.y > ptr[1].v.y)
                        top = ptr + 1;

                    if (top->v.y > ptr[2].v.y)
                        top = ptr + 2;
...
X_NOINLINE void renderLine(int32 x, int32 y, int32 width, int32 height, int32 index, int32 z)
{
    ...
    gVerticesBase[0].x = x;
    gVerticesBase[0].y = y;
    gVerticesBase[0].g = ecsRemap[index];
...
void *copyBackground()
{
    if (ecsDepth && ecsDepth < 8)
    {
        const uint8 *src = (const uint8 *)fb;
        uint8 *dst = gBackgroundCopy;
        int32 count = FRAME_WIDTH * FRAME_HEIGHT;
